...
def build_sql(llm_output: dict, dialect: str) -> list[str]:
    ...
    # PASS 1 — register ALL tables to metadata first
    table_objects = []
    for table in db_schema.tables:
        ...
        table_objects.append(Table(table.name, metadata, *cols))

    # PASS 2 — now compile (all tables known to metadata)
    statements = []
    for table_obj in table_objects:
        statement = CreateTable(table_obj)
        statements.append(str(statement.compile(
            dialect=dialect_map.get(dialect, postgresql.dialect())
        )))
